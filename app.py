import matplotlib
matplotlib.use('Agg') # Use non-interaction backend before importing pyplot
...
def preprocess_comment(comment):
    ...
    try:
        ...
        # Remove trailing and laoding whitespaces 
...
# Load the model and Vectorizer from the Local Machine
...
def predict():
    ...
    try:
        # Preprocess each comment before vectorizer
        preprocessed_comments = [preprocess_comment(comment) for comment in comments]

        # Transform comments using to dense format
        transformed_comments = vectorizer.transform(preprocessed_comments)

        # convert the sparse metrix to dense format
        dense_comments = transformed_comments.toarray()

        # Make Prediction
        predictions = model.predict(dense_comments).tolist()

        # Convert prediction to string for consistency
        # prediction = [str(pred) for pred in prediction]
    
    except Exception as e:
        return jsonify({"Error": f"Prediction Failed: {str(e)}"}), 500
    
    # Return the response with original comments and prediction sentimentss
